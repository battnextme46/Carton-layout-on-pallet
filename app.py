...
st.write("เครื่องมือคำนวณและจำลองการจัดวางกล่องสินค้าบนพาเลทแบบปรับทิศทางได้ 6 รูปแบบ")

# --- SIDEBAR INPUTS ---
st.sidebar.header("1. ขนาดกล่องสินค้า (mm)")
...
st.sidebar.header("2. ขนาดพาเลทและข้อจำกัด (mm)")
...
st.sidebar.header("3. ค่าเผื่อ (mm)")
...
def calculate_pallet_layout(bw_used, bl_used, bh_used, case_name, is_normal_case):
    ...
    max_w, max_l = pallet_w + (2 * overhang_allowance), pallet_l + (2 * overhang_allowance)
    
    slots_w = math.floor(max_w / effective_box_w)
    slots_l = math.floor(max_l / effective_box_l)
    boxes_per_layer = slots_w * slots_l
    
    available_h = max_air_height - pallet_h
    max_layers = math.floor(available_h / bh_used) if available_h > 0 else 0
    total_boxes = boxes_per_layer * max_layers
    total_height = pallet_h + (max_layers * bh_used) if total_boxes > 0 else pallet_h
    
    used_w = slots_w * effective_box_w - box_tolerance
    used_l = slots_l * effective_box_l - box_tolerance
    area_eff = (used_w * used_l) / (pallet_w * pallet_l) * 100 if total_boxes > 0 else 0
    
    return {
        "CASE_NAME": case_name, "TYPE": "ปกติ" if is_normal_case else "ทางเลือก",
        "BW_USED": bw_used, "BL_USED": bl_used, "BH_USED": bh_used,
        "SLOTS_W": slots_w, "SLOTS_L": slots_l, "BOXES_PER_LAYER": boxes_per_layer,
        "MAX_LAYERS": max_layers, "TOTAL_BOXES": total_boxes, "TOTAL_HEIGHT": total_height, "AREA_EFFICIENCY": area_eff
    }
...
def generate_svg_pallet_layer(params, color_theme):
    if params["TOTAL_BOXES"] == 0:
        return '<svg width="100%" height="auto" viewBox="0 0 1000 800"><rect width="1000" height="800" fill="#f8fafc" stroke="#ef4444" stroke-width="4" /><text x="500" y="400" font-size="40" fill="#ef4444" text-anchor="middle">วางไม่ได้</text></svg>'
    
    sw, sl, bw, bl = params["SLOTS_W"], params["SLOTS_L"], params["BW_USED"], params["BL_USED"]
    pad = 150
    view_w, view_h = pallet_w + (pad * 2), pallet_l + (pad * 2)
    svg = f'<svg width="100%" height="auto" viewBox="0 0 {view_w} {view_h}" xmlns="http://www.w3.org/2000/svg" style="background-color: #ffffff; border-radius: 12px; border: 2px solid #e2e8f0;">'
    svg += f'<rect x="{pad}" y="{pad}" width="{pallet_w}" height="{pallet_l}" fill="#fcfbf7" stroke="{color_theme}" stroke-width="6" />'
    ...
    for i in range(sw):
        for j in range(sl):
            ...
            svg += f'<rect x="{x}" y="{y}" width="{bw}" height="{bl}" fill="#ffedd5" stroke="#ea580c" stroke-width="2" />'
            if i==0 and j==0:
                svg += f'<text x="{x+bw/2}" y="{y+bl/2-5}" font-size="18" font-weight="bold" fill="#9a3412" text-anchor="middle">W:{int(bw)}</text>'
                svg += f'<text x="{x+bw/2}" y="{y+bl/2+20}" font-size="18" font-weight="bold" fill="#9a3412" text-anchor="middle">L:{int(bl)}</text>'
    svg += '</svg>'
    return svg
...
with col1:
    st.subheader("🟢 แบบที่ 1: จัดเรียงแบบปกติ (Normal Case)")
    res = normal_cases[0]
    m1, m2, m3 = st.columns(3)
    m1.metric("จำนวนทั้งหมด", f"{res['TOTAL_BOXES']} ใบ")
...
with col2:
    ...
    m4.metric("จำนวนทั้งหมด", f"{res['TOTAL_BOXES']} ใบ")
...
if alt_cases[0]["TOTAL_BOXES"] > normal_cases[0]["TOTAL_BOXES"]:
    st.success(f"🔥 ทางเลือกอื่นๆ (Alternative) จุได้มากกว่าปกติ {alt_cases[0]['TOTAL_BOXES'] - normal_cases[0]['TOTAL_BOXES']} ใบ!")
